...
import {
  PlanCreationInputSchema,
  PlanCreationOutput,
  planCreationWorkflow,
} from './workflows/planCreationWorkflow';
 ...
 * - /plan/stream: Server-sent events (SSE) endpoint for plan creation
 */
  ...
  registerApiRoute('/plan/stream', {
    ...
    openapi: {
      requestBody: {
        content: {
          'application/json': {
            schema: toOpenApiSchema(PlanCreationInputSchema),
          },
        },
      },
    },
    handler: async (c) => {
      try {
        ...
        const {
          ...
          resourceId,
          threadId,
        } = PlanCreationInputSchema.parse(body);
